...
size_t changeset_serialized_size(const changeset_t *ch)
{
	...
	changeset_iter_t it;
	changeset_iter_all(&it, ch, false);
...
int changeset_serialize(const changeset_t *ch, uint8_t *dst, size_t size)
{
	...
	wire_ctx_t wire = wire_ctx_init(dst, size);

	/* Serialize SOA 'from'. */
	int ret = serialize_rrset(&wire, ch->soa_from);
	if (ret != KNOT_EOK) {
		return ret;
	}
	...
	knot_rrset_t rrset = changeset_iter_next(&it);
	while (!knot_rrset_empty(&rrset)) {
		ret = serialize_rrset(&wire, &rrset);
		if (ret != KNOT_EOK) {
			changeset_iter_clear(&it);
			return ret;
		}
	...
	/* Serialize SOA 'to'. */
	ret = serialize_rrset(&wire, ch->soa_to);
	if (ret != KNOT_EOK) {
		return ret;
	}

	/* Serialize RRSets from the 'add' section. */
	ret = changeset_iter_add(&it, ch, false);
	if (ret != KNOT_EOK) {
		return ret;
	}
...
int changeset_deserialize(changeset_t *ch, const uint8_t *src, size_t size)
{
	...
	wire_ctx_t wire = wire_ctx_init_const(src, size);
	...
	while (wire_ctx_available(&wire) > 0) {
		...
		if (rrset.type == KNOT_RRTYPE_SOA) {
			...
			if (in_remove_section) {
				ret = changeset_rem_rrset(ch, &rrset, 0);
			} else {
				ret = changeset_add_rrset(ch, &rrset, 0);
			}
		}
