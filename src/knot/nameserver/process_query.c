...
static int put_rrsig(struct query_data *qdata, const knot_dname_t *sig_owner,
                     uint16_t type, const knot_rrset_t *rrsigs, knot_rrinfo_t *rrinfo)
{
                         ...
                         uint16_t compr_hint, uint32_t flags)
{
	...
	if (pkt->current == KNOT_ANSWER) {
		...
		expand = (knot_dname_is_wildcard(rr->owner) &&
		         !knot_dname_is_wildcard(qdata->name));
	}
	...
	if (inserted &&
		...
		ret = put_rrsig(qdata, rr->owner, rr->type, rrsigs, rrinfo);
	}
