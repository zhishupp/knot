...
static int store_changesets(journal_t *j, list_t *changesets)
{
	...
	changeset_t /* *dbgchsh = HEAD(*changesets), */ *dbgchst = TAIL(*changesets);
	//fprintf(stdout, "inserting %zu changesets %u -> %u (%d)\n", (size_t)list_size(changesets), knot_soa_serial(&dbgchsh->soa_from->rrs), knot_soa_serial(&dbgchst->soa_to->rrs), txn->ret);
	//journal_check(j, KNOT_JOURNAL_CHECK_INFO);
...
static int open_journal_db_unsafe(journal_db_t **db)
{
	...
	opts.path = (*db)->path;
	opts.mapsize = ((*db)->fslimit == KNOT_JOURNAL_FSLIMIT_SAMEASLAST ? 1024 * 1024 * 1024 : (*db)->fslimit);
	opts.maxdbs = 1;
	...
	size_t last_fslimit = knot_db_lmdb_get_mapsize((*db)->db);

	if ((*db)->fslimit == KNOT_JOURNAL_FSLIMIT_SAMEASLAST) {
		(*db)->fslimit = last_fslimit;
		(*db)->db_api->deinit((*db)->db);
		(*db)->db = NULL;
		return open_journal_db_unsafe(db);
	}

	// Check if we by any chance opened the DB with smaller mapsize than before.
	// If so, we need to return error, flush and retry.
	if (last_fslimit > (*db)->fslimit) {
		log_warning("reopening journal DB with smaller mapsize %zu versus %zu\n", knot_db_lmdb_get_mapsize((*db)->db), (*db)->fslimit);
		(*db)->db_api->deinit((*db)->db);
		(*db)->db = NULL;
		if (!remove_path((*db)->path)) {
			return KNOT_ERROR;
		}
		return open_journal_db_unsafe(db);
	}
