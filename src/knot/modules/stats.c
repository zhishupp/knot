...
static int count(int state, knot_pkt_t *pkt, struct query_data *qdata, void *ctx)
{
	...
	if (stats->rcode) {
		if (xfr_packets <= 1) {
			if (qdata->rcode_tsig == KNOT_RCODE_BADSIG) {
				mod_ctrs_incr(stats->counters, CTR_RCODE, RCODE_BADSIG, 1);
			} else {
				mod_ctrs_incr(stats->counters, CTR_RCODE, rcode, 1);
			}
		} else if (rcode != KNOT_RCODE_NOERROR) {
			// Count failed transfer last message RCODE.
			mod_ctrs_decr(stats->counters, CTR_RCODE, KNOT_RCODE_NOERROR, 1);
			mod_ctrs_incr(stats->counters, CTR_RCODE, rcode, 1);
		}
	}
	...
	if (stats->nodata && rcode == KNOT_RCODE_NOERROR &&
...
int stats_unload(struct query_module *self)
{
	...
	stats_t *stats = (stats_t *)self->ctx;
