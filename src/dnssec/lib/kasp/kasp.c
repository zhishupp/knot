			    ...
			    dnssec_kasp_keyusage_t **keyusage_ptr)
{
	...
	dnssec_kasp_keyusage_t *keyusage = dnssec_kasp_keyusage_new();
	if (!keyusage) {
		return DNSSEC_ENOMEM;
	}
...
int dnssec_kasp_keyusage_save(dnssec_kasp_t *kasp, const dnssec_kasp_keyusage_t *keyusage)
{
	...
	return kasp->functions->keyusage_save(kasp->ctx, keyusage);
}
...
int dnssec_kasp_keyusage_remove(dnssec_kasp_t *kasp)
{
	...
	return kasp->functions->keyusage_remove(kasp->ctx);
}
...
int dnssec_kasp_keyusage_exists(dnssec_kasp_t *kasp)
{
	...
	return kasp->functions->keyusage_exists(kasp->ctx);
}
