...
static int init(knot_db_t **db_ptr, knot_mm_t *mm, void *arg)
{
	...
	if (old_env == NULL) {
		...
		env->env = old_env->env;
		env->shared = true;
		//old_env->shared = true; // but you SHALL deinit the original DB as last one
	}

	/* Open the database. */
	int ret = dbase_open(env, (struct knot_db_lmdb_opts *)arg);
	if (ret != KNOT_EOK) {
		mm_free(mm, env); // FIXME: mistake here? how about if (!env->shared)
		return ret;
	}
...
_public_
float knot_db_lmdb_get_usage(knot_db_t * db)
{
	struct lmdb_env *env = db;
	knot_db_txn_t txn;
	...
	float pgs_used = st.ms_branch_pages + st.ms_leaf_pages + st.ms_overflow_pages + st.ms_entries;
	float mapsize = (float) knot_db_lmdb_get_mapsize(db);
	if (mapsize == 0.0f) return -2.0f;

	return (pgs_used * st.ms_psize) / mapsize;
}
