...
check_item(knot, "mod-stats", "request-protocol", 1, "tcp%s" % proto, zone=zones[0])
...
# Check response code metrics (non-QUERY rcodes per zone are not counted!).
check_item(knot, "mod-stats", "response-code", 5, idx="NOERROR")
check_item(knot, "mod-stats", "response-code", 2, idx="NOERROR", zone=zones[0])
