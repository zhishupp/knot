...
int __wrap_log_msg_zone(int priority, const knot_dname_t *zone, const char *fmt, ...){
...
int setup(void **state)
{
	...
	*state = malloc(sizeof(group_context_t));
	if (state == NULL)
		return 1;

	group_context_t *ctx = *state;
	...
	if (zone_events_init(&ctx->zone) != KNOT_EOK)
		return 3;
	if (zone_events_setup(&ctx->zone ,ctx->pool, &ctx->sched, NULL) != KNOT_EOK)
		return 4;
	return 0;
}
